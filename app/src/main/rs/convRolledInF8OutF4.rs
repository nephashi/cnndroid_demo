        ...
        for (int w = 0 ; w < w_k ; w++){
            for (int i = 0 ; i < c_k_new / 2 ; i++)
            {
                ...
                else
                {
                    ...
                    int kernel_index = kernel_num * 4 * kernel_size +  h * w_k * c_k_new + w *  c_k_new + 2 * i;
                    ...
                    kernel_value1 = rsGetElementAt_float4(Kernel_Blob,kernel_index);
                    kernel_value2 = rsGetElementAt_float4(Kernel_Blob,kernel_index + 1);
                    sum.z += dot(frame_value1 ,kernel_value1) + dot(frame_value2 ,kernel_value2);

                    kernel_index += kernel_size;
                    kernel_value1 = rsGetElementAt_float4(Kernel_Blob,kernel_index);
                    kernel_value2 = rsGetElementAt_float4(Kernel_Blob,kernel_index + 1);
                    sum.w += dot(frame_value1 ,kernel_value1) + dot(frame_value2 ,kernel_value2);
                }
            }
        }
    }
